...
class Scanner {
    ...
    scanRemote(body){
        ...
        let targetInputs = body.targetInputs;

        let results = [];
        // Fuzz Test!
        for(let i=0; i<fuzzTests.length; i++){
            console.log("Running test: " + fuzzTests[i] + " on " + url);
            results.push(this.sendScan(fuzzTests[i], url, successContent, targetInputs));
        }
        return Promise.all(results);

    }

    sendScan(fuzz, url, successContent, targetInputs){
        
        var request = require('request');

        var bodyObj = {};
        console.log(targetInputs);
        var targetInputs = targetInputs.split(",");
        for(let i=0; i<targetInputs.length; i++){
            bodyObj[targetInputs[i]] = fuzz;
        }
